...
describe('createSigV4Fetch', () => {
  it('signs the request and passes signed headers to globalThis.fetch', async () => {
    ...
    const sigFetch = createSigV4Fetch({ region: 'us-east-1', credentials: creds });
    ...
    expect(mockSign).toHaveBeenCalledWith(
      expect.objectContaining({
        ...
        path: '/v1/responses',
        headers: expect.objectContaining({ 'content-type': 'application/json', host: 'bedrock.us-east-1.amazonaws.com' }),
        body: '{"input":"hi"}',
      }),
    );
    expect(fetchSpy).toHaveBeenCalledWith(
      'https://bedrock.us-east-1.amazonaws.com/v1/responses',
      expect.objectContaining({ headers: { authorization: 'AWS4-HMAC-SHA256 ...' } }),
    );
  });
  ...
  it('defaults method to GET when not provided', async () => {
    ...
    const sigFetch = createSigV4Fetch({ region: 'us-west-2', credentials: creds });
    await sigFetch('https://example.com/path');

    expect(mockSign).toHaveBeenCalledWith(expect.objectContaining({ method: 'GET' }));
  });
